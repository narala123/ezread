 ...
 */
@Component({
  selector: 'phonicimages',
  templateUrl: 'phonicimages.html',
  styleUrls: ['/src/components/phonicimages/phonicimages.scss'],
  providers: [dBconnectionService]
})
export class PhonicimagesComponent {
...
//console.log(this.alertCtrl);
    ...
    for (var i = 0; i < this.phonicImages.length; i++) {
      ...
      this.phonicImages[i].splitwords = this.phonicImages[i]._id.split('');
...
console.log(this.bookmarks);
  ...
  listen(word) {
    ...
    this.tts.speak({text: word, locale: 'en-GB', rate: 0.75})
      .then(() => console.log('Success'))
      .catch((reason:any) => console.log(reason));

   /* for (var i = 0; i < wordSplit.length; i++) {
      this.tts.speak({text: wordSplit[i], locale: 'en-GB', rate: 0.50})
        .then(() => console.log('Success'))
        .catch((reason:any) => console.log(reason));
    }*/
